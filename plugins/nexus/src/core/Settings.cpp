...
namespace cba
{
	...
	Settings Settings::Load(const std::string& aAddonDir)
	{
		...
		while (std::getline(file, line))
		{
			...
			else if (key == "AutoBrightSource")  s.AutoBrightnessSource = std::clamp(safeStoi(value, 0), 0, 2);
			else if (key == "SensorTarget")      s.SensorBrightnessTarget = std::clamp(safeStof(value, 0.0f), 0.0f, 1.0f);
	...
	void Settings::Save(const std::string& aAddonDir) const
	{
		...
		file << "AutoBrightSource=" << AutoBrightnessSource << "\n";
		file << "SensorTarget=" << SensorBrightnessTarget << "\n";
	...
	{
		// Built field by field, NOT as one positional format string
		// (2026-09-11). It used to be a single snprintf whose placeholder
		// order and argument list had to be kept in sync by hand - the same
		// silent-field-shift hazard as L10n.h's positional language blocks,
		// where a mistake produces a plausible-looking but wrong value and the
		// compiler says nothing. Here each key is written next to its own
		// value, so the class of bug simply cannot occur and the roundtrip
		// check that guards it drops from load-bearing to nice-to-have.
		//
		// The importer has always been key-based, so field order in this
		// string is irrelevant and unknown keys are ignored - both directions
		// stay compatible with older and newer builds.
		//
		// SE (SmartEnhancer) is gone entirely as of 2026-09-11 - the field had
		// no behavioural reader left, so neither writing nor reading it meant
		// anything. A code from an older build that still carries SE imports
		// fine: unknown keys are ignored, which is also why adding keys here
		// never breaks older readers.
		std::string out = "CBA1:";
		auto addInt = [&out](const char* aKey, int aValue) {
			char f[32];
			std::snprintf(f, sizeof(f), "%d", aValue);
			out += aKey; out += "="; out += f; out += ";";
		};
		auto addFloat = [&out](const char* aKey, float aValue) {
			char f[32];
			std::snprintf(f, sizeof(f), "%.2f", aValue);
			out += aKey; out += "="; out += f; out += ";";
		};

		addInt  ("T",   static_cast<int>(Type));
		addFloat("S",   static_cast<float>(Severity01));
		addInt  ("M",   Mixed ? 1 : 0);
		addFloat("RG",  static_cast<float>(MixedRgSeverity01));
		addFloat("BY",  static_cast<float>(MixedBySeverity01));
		addFloat("G",   GammaGain);
		addInt  ("CT",  CommanderTagMode);
		addFloat("ET",  EnhancerTolerance);
		addInt  ("CLP", CommanderTagLayerPriority);
		addInt  ("EM",  EyeComfortModeEnabled ? 1 : 0);
		addFloat("BF",  BlueFilter01);
		addFloat("WT",  WarmTint01);
		addFloat("SR",  SaturationReduction01);
		...
		while (std::getline(ss, item, ';'))
		{
			...
			try
			{
				if (k == "T") {
					...
					// Filter Layer Matrix rank for the Commander Tag layer.
					// Clamped defensively: this arrives from a pasted string,
					// and a wild value would put the layer at an unreachable
					// end of GetFilterLayerOrder()'s sort.
					CommanderTagLayerPriority = std::clamp(std::stoi(v), -99, 99);
					++appliedCount;
				} else if (k == "H") {
					DiagnosisHint = v;
					++appliedCount;
