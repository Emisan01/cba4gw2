...
namespace cba
{
	...
	{
		char buf[256];
		// POSITIONAL format string - the placeholder order and the argument
		// list below must be edited together, exactly like L10n.h's language
		// blocks. Two changes made 2026-09-11, both in lockstep:
		//   - dropped SE (SmartEnhancer): the field has had no behavioural
		//     reader since the Auto-Com-Tag selectivity fix, so a shared code
		//     was carrying dead weight. Import still ACCEPTS SE, so codes
		//     generated by older builds keep working - the asymmetry is
		//     deliberate.
		//   - added CLP (CommanderTagLayerPriority): its sibling ET
		//     (EnhancerTolerance) was already here, so a shared profile used
		//     to transport one half of the Commander Tag layer setup and
		//     silently drop the other.
		// Unknown keys are ignored by the importer, so both directions stay
		// compatible with other builds.
		std::snprintf(buf, sizeof(buf), "CBA1:T=%d;S=%.2f;M=%d;RG=%.2f;BY=%.2f;G=%.2f;CT=%d;ET=%.2f;CLP=%d;EM=%d;BF=%.2f;WT=%.2f;SR=%.2f",
