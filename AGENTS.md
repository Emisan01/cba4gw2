...
Companion to `CLAUDE.md` (project-wide rules, architecture, safety) — that file
is not duplicated here. This one is UI/code convention detail; `CLAUDE.md` is
the terse rule list loaded into every session.
...
- **Rule**: Any UI button or toggle that arms or switches a filter mode (Commander-Tag profile presets, the master `Enabled` toggle, `Reset to Neutral`) must invoke:
  ```cpp
  EnsureDeferredInitialized();
  // Set settings flags ...
  Recompute(/*aForce=*/true);
  ```
- This guarantees that cold-start initialization lag is eliminated and the active backend (Shader or DWM) is updated immediately without requiring the user to touch other settings first.

---

## 4. Game Memory Safety & ArenaNet Policy Compliance

- **Rule**: CBA is strictly a visual presentation-layer assistant (pixel shader on GW2's own backbuffer by default, Windows Magnification DWM API as the complementary screen-wide path).
...
- Maintain the official policy notice in README and the About panel (System tab):
